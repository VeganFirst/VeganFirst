	...
	<script src="<?php echo base_url()."assets/"; ?>js/application.min.js"></script>
        <script>
            function confirmDelete(){
                var agree = confirm("Are you sure to delete this Author?");
                if (agree){
                    return true;
                }else{
                    return false;
...
<body>
	...
	<div class="container-fluid" id="content">
			...
			$total=  $this->M_Article->getArticleCountbyAuthor($Writer->id)+$this->M_Recipes->getArticleCountbyAuthor($Writer->id);
											
															
														if($Writer->isActive==1)
														{
														$status ="Active";
														}
														else
							 ...
							 echo "<tr>
							   ...
							   <td>{$status}</td>
							   <td>{$total}</td>
                               <td><a href='".  base_url()."admin/edit_author/{$Writer->id}'>Edit</a> | <a href='".  base_url()."admin/delete_author/{$Writer->id}' onclick='return confirmDelete();'>Delete</a></td>
                                                        </tr>";
                                                        }
                                                    }
                                                    ?>
