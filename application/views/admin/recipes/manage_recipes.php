	...
	<script src="<?php echo base_url()."assets/"; ?>js/application.min.js"></script>
        <script>
            function confirmDelete(){
                var agree = confirm("Are you sure to delete this Recipe?");
                if (agree){
                    return true;
                }else{
                    return false;
...
<body>
	...
	<div class="container-fluid" id="content">
                                        ...
                                        <div class="box box-color box-bordered">
                                            <div class="box-title">
                                                    ...
                                                    <h3 class="pull-right" style="margin-right:20px;">
                                                         <a href="<?php echo base_url().'admin/ExportRecipes'?>">  <i class="icon-download-alt"></i>Export Csv</a>
                                                    </h3>
                                                    
                                                    <h4 class="text-right" style="padding-right: 10px;"><a href="add_recipes" style="color: #fff;">Add New Recipes</a></h4>
                                            </div>
                                            <div class="box-content nopadding table-responsive ">
                                                <table class="table table-hover table-nomargin dataTable table-bordered">
                                  <thead>
                                  <tr>
                                  <th width="30px;">Recipe Id</th>

                                   <th>Recipe Name</th>
                                   <th>Recipe By</th>
                                   <th>Category</th>
                                   <th width="60px;">Tags</th>
                                   <th width="60px;">Aded on</th>
                                   <th width="60px;">updated on</th>
           ...
           foreach ($Articles as $Article){
			   $iurl= base_url()."media/upload/recipes/";
			   $cat=$this->M_Category->getCategoryInfo_rec($Article->Category);
			   if($Article->isActive==1){$status ="Yes";}else { $status ="No";}
			   
           echo "<tr>
		   <td>{$Article->idRecepie}</td>
		   ...
		   <td>{$Article->Views}</td>
                                                                                              <td>{$status}</td>
		   <td ><a href='".  base_url()."recipe/{$Article->Url}' target='_blank'>View</a> | <a href='".  base_url()."admin/edit_recipes/{$Article->idRecepie}'>Edit</a> | <a href='".  base_url()."admin/delete_recipes/{$Article->idRecepie}' onclick='return confirmDelete();'>Delete</a></td>
                                                        </tr>";
                                                        }
                                                    }
                                                    ?>
