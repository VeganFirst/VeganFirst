...
jQuery.ajax({
  ...
  success: function(returndata) {
    if (returndata.id==1) {
		...
		$('#flo'+e).html("<i class='material-icons'>done</i> FOLLOWED TOPIC");
    } else {
      	$("#login_popup").modal('show');
    }
...
jQuery.ajax({
  ...
  success: function(returndata) {
    if (returndata.id==1) {
		...
		$('#flo'+e).html("<i class='material-icons'>add</i> FOLLOW TOPIC");
    } else {
      	$("#login_popup").modal('show');
    }
...
jQuery.ajax({
  ...
  success: function(returndata) {
    if (returndata.id==1) {
		...
		//$('#bookmark'+e).attr('data-original-title', returndata.message).tooltip('show');
...
jQuery.ajax({
  ...
  success: function(returndata) {
    ...
    if (returndata.id==1) {
		...
		$('#bookm'+e).css("color", "#fda58d");

		$('#bookm'+e).attr('data-original-title', returndata.message).tooltip('show');*/
...
function ValidateAdvt() {
	var z = document.forms["Advt"]["Email"].value;
	var y = document.forms["Advt"]["Name"].value;
	var s = document.forms["Advt"]["phone"].value;
	var m = document.forms["Advt"]["message"].value;
	var atpos = z.indexOf("@");
    var dotpos = z.lastIndexOf(".");
	var msg = [];
	if (y == null || y == "") {
	...
	if (s == null || s == "") {
        msg.push("<div class='alert alert-danger'>Enter Phone</div>");
    }
	if (m == null || m == "") {
        msg.push("<div class='alert alert-danger'>Enter Your Message</div>");
    }
